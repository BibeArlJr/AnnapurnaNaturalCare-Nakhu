...
export default function CourseBookingModal({ course, open, onClose }) {
  ...
  const price = Number(form.people || 1) * (Number(course.price) || 0);
  ...
  const submit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const res = await apiPost("/course-bookings", {
        courseId: course._id,
      ...
      const id = res?.data?._id || res?._id || null;
      setSubmittedId(id);
      if (id) setShowPaymentChoice(true);
    } catch (err) {
  ...
  const payWithStripe = async () => {
    ...
    try {
      const res = await apiPost("/payments/stripe/checkout", { bookingId: submittedId, type: "course" });
      if (res?.url) {
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-start justify-center px-4 py-10" onClick={close}>
      ...
      >
        <div className="flex items-start justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-teal-600 font-semibold">Book this course</p>
            <h3 className="text-xl font-semibold text-slate-900">{course.title}</h3>
            <p className="text-sm text-slate-600">{course.duration ? `${course.duration} days` : ""}</p>
          </div>
        ...
        {showPaymentChoice && submittedId ? (
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border border-green-200 bg-green-50 px-4 py-3 text-green-800">
              ...
              <p className="text-sm text-green-700">Our team will confirm shortly. Reference: {submittedId}</p>
            </div>
            <div className="flex justify-end">
              <button
                onClick={close}
                className="inline-flex items-center justify-center px-5 py-2 rounded-full bg-teal-600 text-white text-sm font-semibold hover:bg-teal-700 transition"
              >
                Close
              </button>
            </div>
          ...
          <form className="space-y-4" onSubmit={submit}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {["online", "residential"].map((mode) => (
                  ...
                  >
                    ...
                    <p className="text-sm text-slate-600">USD {course.price || 0}</p>
                  </label>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
              <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                ...
                <p className="font-semibold text-slate-900">${course.price || 0}</p>
              </div>
              <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                <p className="text-slate-600">People</p>
                <p className="font-semibold text-slate-900">{form.people}</p>
