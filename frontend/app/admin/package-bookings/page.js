...
const statusClasses = {
  ...
  rescheduled: "bg-blue-900/60 text-blue-200",
...
export default function PackageBookingsAdminPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        console.error("Package bookings load error:", err);
      } finally {
        setLoading(false);
      }
    }
  ...
  const startEdit = (item) => {
    ...
    setForm({
      ...
      peopleCount: item.peopleCount || item.numberOfPersons || 1,
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await apiPut(`/package-bookings/${editing._id}`, form);
      const refreshed = await apiGet("/package-bookings");
  ...
  const remove = async (id) => {
    ...
    try {
      await apiDelete(`/package-bookings/${id}`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-white">Package Bookings</h1>
        {editing && (
          <div className="flex items-center gap-2 text-sm text-slate-300">
            <PencilSquareIcon className="h-5 w-5" />
            Editing booking for {editing.userName || editing.name}
          </div>
        )}
      </div>
      ...
      {loading ? (
        ...
        <p className="text-slate-400 text-sm">No package bookings yet.</p>
      ) : (
        <div className="overflow-hidden border border-slate-800 rounded-xl bg-slate-900">
          ...
          {bookings.map((item) => {
            const pkg = packageMap[item.packageId] || packageMap[item.packageSlug];
            const total = item.totalAmount || item.totalAmountUSD || item.totalPriceUSD;
            return (
              <div
                key={item._id}
              ...
              >
                ...
                <div className="col-span-2">
                  <p className="text-sm text-slate-200">{item.packageName || pkg?.name || "—"}</p>
                </div>
          ...
          <div className="w-full max-w-lg bg-[#0f131a] border border-white/10 rounded-2xl shadow-2xl p-0 flex flex-col max-h-[90vh]">
            ...
            <div className="px-6 py-4 border-t border-white/10 space-y-2">
              ...
              <div className="text-xs text-slate-400 flex items-center gap-1">
                <CheckCircleIcon className="h-4 w-4 text-green-400" /> Confirms booking
                <XCircleIcon className="h-4 w-4 text-red-400 ml-2" /> Cancels with reason
                <ArrowPathIcon className="h-4 w-4 text-yellow-400 ml-2" /> Reschedules with note
              </div>
            </div>
          </div>
        </div>
