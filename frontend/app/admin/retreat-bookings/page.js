...
export default function AdminRetreatBookingsPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await apiGet("/retreat-bookings");
      setBookings(res?.data || res || []);
    } catch (err) {
      console.error("Retreat bookings load error:", err);
    } finally {
      setLoading(false);
    }
  }
  ...
  const startEdit = (item) => {
    ...
    setForm({
      ...
      retreatPricePerPerson: item.retreatPricePerPerson || "",
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await apiPut(`/retreat-bookings/${editing._id}`, form);
      await load();
      setEditing(null);
      setForm({
        status: "pending",
  ...
  const remove = async (id) => {
    ...
    try {
      await apiDelete(`/retreat-bookings/${id}`);
      await load();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-white">Retreat Bookings</h1>
        {editing && (
      ...
      {loading ? (
        ...
        <p className="text-slate-400 text-sm">No retreat bookings yet.</p>
      ) : (
        <div className="overflow-hidden border border-slate-800 rounded-xl bg-slate-900">
          ...
          {bookings.map((item) => (
            ...
            >
              ...
              <div className="col-span-2">
                <p className="text-sm text-slate-200">{item.programTitle}</p>
              </div>
              <div className="col-span-2 text-slate-200">
                {item.totalAmountUSD ? `$${item.totalAmountUSD}` : "—"}
              </div>
          ...
          <div className="w-full max-w-lg bg-[#0f131a] border border-white/10 rounded-2xl shadow-2xl p-0 flex flex-col max-h-[90vh]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="space-y-2">
                ...
                <input
                  type="number"
                  value={form.retreatPricePerPerson}
                  onChange={(e) => setForm((prev) => ({ ...prev, retreatPricePerPerson: Number(e.target.value) || 0 }))}
                  className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-teal-500"
                />
              </div>
            </div>
