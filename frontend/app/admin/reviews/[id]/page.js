...
export default function EditReviewPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await apiGet(`/patient-reviews/${id}`);
        ...
        setForm({
          ...
          associatedProgram: data?.associatedProgram || "",
        });
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await apiPatch(`/patient-reviews/${id}`, { ...form, coverImage, images, videoCoverImage, videoUrl });
    ...
    <div className="flex justify-center">
      ...
      >
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-white">Edit Patient Review</h1>
            <p className="text-sm text-slate-400">Update testimonial content.</p>
          </div>
          <button
            type="button"
            onClick={() => router.back()}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          />
          <Field label="Rating (1-5)" name="rating" type="number" min="1" max="5" value={form.rating || 5} onChange={handleChange} />
        ...
        <div className="space-y-4 border border-slate-800 rounded-xl p-4 bg-slate-900/60">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <MediaCard title="Cover image" helper="Single image shown as the hero thumbnail.">
              <UploadBox
            ...
            <MediaCard title="Video testimonial" helper="Paste a YouTube/Vimeo link or upload an MP4 (max 200MB).">
            ...
            <MediaCard title="Gallery images" helper="Upload up to 10 images. Drag to reorder is not required.">
              <UploadBox
                accept="image/png,image/jpeg,image/webp"
                multiple
                onChange={(e) => {
            ...
            {(form.videoUrl || videoUpload) && (
              <MediaCard title="Video cover image" helper="Shown as poster before playing the video.">
                <UploadBox
                  accept="image/png,image/jpeg,image/webp"
