...
export default function HealthProgramBookingsAdminPage() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
        const [bookingsRes, programRes] = await Promise.all([apiGet("/health-program-bookings"), apiGet("/health-programs")]);
        setBookings(bookingsRes?.data || bookingsRes || []);
        setPrograms(programRes?.data || programRes || []);
      } catch (err) {
        console.error("Health program bookings load error:", err);
      } finally {
        setLoading(false);
      }
    }
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await apiPut(`/health-program-bookings/${editing._id}`, form);
      const refreshed = await apiGet("/health-program-bookings");
  ...
  const remove = async (id) => {
    ...
    try {
      await apiDelete(`/health-program-bookings/${id}`);
      const refreshed = await apiGet("/health-program-bookings");
      setBookings(refreshed?.data || refreshed || []);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-white">Health Program Bookings</h1>
        {editing && (
      ...
      {loading ? (
        ...
        <p className="text-slate-400 text-sm">No health program bookings yet.</p>
      ) : (
        <div className="overflow-hidden border border-slate-800 rounded-xl bg-slate-900">
          ...
          {bookings.map((item) => {
            const program = programMap[item.programId] || programMap[item.programSlug];
            ...
            return (
              ...
              >
                ...
                <div className="col-span-2">
                  <p className="text-sm text-slate-200">{item.programTitle || program?.title || "—"}</p>
